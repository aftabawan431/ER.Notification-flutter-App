...
class AuthServices {
  ...
  Future<UserModal?> checkIfUserLoggedIn() async {
  ...
  Future<void> loginUser(
    ...
    if (email.toLowerCase() == 'admin@gmail.com') {
      ...
      if (admin == null) {
        final newUser =
            UserModal(userId: DateTime.now().toString(), name: 'admin', designation: 'Manager', email: 'admin@gmail.com', mobile: '03407853962', isActive: true, isAdmin: true);
        ...
        final user = UserModal.fromJson(jsonDecode(admin));
        await _secureStorage.write(key: 'user', value: jsonEncode(user.toJson()));
        setCurrentUser(context, user);
      }
    } else {
      final user = context.read<UserProvider>().users.firstWhere((element) => element.email == email);
  ...
  setCurrentUser(BuildContext context, UserModal user) {
    context.read<UserProvider>().setCurrentUser(user);
  }
